...
tool_init(rocprofiler_client_finalize_t fini_func, void* tool_data)
{
    ...
    if( RPSDK_MODE_DEVICE_SAMPLING == get_profiling_mode() ){
        ...
        for(auto g_it=gpu_agents.begin(); g_it!=gpu_agents.end(); ++g_it){
            ...
            // Must create the context before configuring it.
            ...
            // Must create the the buffer for the context before configuring the context.
        ...
        ROCPROFILER_CALL(rocprofiler_configure_callback_dispatch_counting_service_FPTR(
                             client_ctx, dispatch_callback, tool_data, record_callback, tool_data), // may need to pass by ref?
   ...
   if( it->second[gpu] == 1 ) {            // section
    ...
    if( 0 != (active_event_set_ctx->state & RPSDK_AES_RUNNING) ){
        ret_val = rocprofiler_sample_device_counting_service_FPTR(
                _dev_ctx_[gpu], {}, ROCPROFILER_COUNTER_FLAG_NONE, // may need to pass by ref?
